...
public class PsychicWandItem extends Item {
    public PsychicWandItem(Properties props) {
    ...
    public void onUseTick(Level level, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if (getUseDuration(stack, user) - remainingUseTicks == 1) {
            level.playSound(null, user.getX(), user.getY(), user.getZ(),
                    SoundEvents.EVOKER_CAST_SPELL, SoundSource.PLAYERS, 3.0F, 1.0F);
            level.playSound(null, user.getX(), user.getY(), user.getZ(),
    ...
    private void spawnIceBeam(Level level, LivingEntity user) {
        level.playSound(null, user.getX(), user.getY(), user.getZ(),
                SoundEvents.SHULKER_BULLET_HIT, SoundSource.PLAYERS, 5.0F, 1.0F);
        level.playSound(null, user.getX(), user.getY(), user.getZ(),
        ...
        for (int i = 1; i < steps; i++) {
            ...
            if (level instanceof ServerLevel sl) {
                sl.sendParticles(AwesomeParticles.PSYCHIC_BEAM.get(), p.x, p.y, p.z, 1, 0, 0, 0, 0);
            }
